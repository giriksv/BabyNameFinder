...
class _BoypageWidgetState extends State<BoypageWidget> {
  ...
  int _currentIndexmale = 0; // Initialize with a default value
  ...
  Future<void> _loadCurrentIndexmale() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() {
      _currentIndexmale = prefs.getInt('currentIndexmale') ?? 0;
    });
  }

  Future<void> _saveCurrentIndexmale(int indexmale) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setInt('currentIndexmale', indexmale);
  }
  ...
  void _navigateToNextName() {
    setState(() {
      _currentIndexmale++;
      _saveCurrentIndexmale(_currentIndexmale);
    });

    if ((_currentIndexmale + 1) % 10 == 0) {
  ...
  void _navigateToPreviousName() {
    setState(() {
      _currentIndexmale--;
      if (_currentIndexmale < 0) {
        _currentIndexmale = 0; // Ensure index doesn't go below 0
      }
      _saveCurrentIndexmale(_currentIndexmale);
    });
    ...
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          final name = data[_currentIndexmale]['name'];
          final meaning = data[_currentIndexmale]['meaning'];
                                ...
                                children: <Widget>[
                                  GestureDetector(
                                    onTap: () async {
                                      ...
                                      final currentName = data[_currentIndexmale];
