...
class _NotsurepageWidgetState extends State<NotsurepageWidget> {
  ...
  void initState() {
    super.initState();
    _nameData = _loadNameDataFromCSV(); // Load data from CSV in Firebase Storage
    _loadInterstitialAd();
    _loadCurrentIndex();
  }
  ...
  Future<List<Map<String, dynamic>>> _loadNameDataFromCSV() async {
    try {
      ...
      Reference ref = FirebaseStorage.instance.ref().child('firestorebabynames.csv');
      ...
      for (var row in csvTable) {
        if (row[3] == 'X' && row[2] == nationality) { // Check gender column for 'X'
          dataList.add({
            'name': row[0],
            'meaning': row[1],
            'nationality': row[2],
  ...
  void _loadInterstitialAd() {
    InterstitialAd.load(
      adUnitId: 'ca-app-pub-3940256099942544/1033173712', // Replace with your actual AdMob ad unit ID
  ...
  Future<void> _saveLikedNameToFirestore(String name, String meaning) async {
    try {
      ...
      if (udid != null) {
        ...
        print('Checking if name exists in Firestore: $name for user: $hashedUdid');
        ...
        if (existingNames.docs.isEmpty) {
          ...
          await _firestore.collection('users').doc(hashedUdid).collection('liked_names').add({
            'name': name,
            'meaning': meaning,
          });
  ...
  Future<void> _saveRejectdNameToFirestore(String name, String meaning) async {
    try {
      ...
      if (udid != null) {
        ...
        if (existingNames.docs.isEmpty) {
          // Name does not exist, save it to Firestore
          print('Saving rejected name to Firestore: $name for user: $hashedUdid');
          await _firestore.collection('users').doc(hashedUdid).collection('rejected_names').add({
            'name': name,
            'meaning': meaning,
          });
                                ...
                                children: <Widget>[
                                  GestureDetector(
                                    onTap: () async {
                                      ...
                                      await _saveLikedNameToFirestore(name, meaning);
                                      setState(() {
                                  ...
                                  GestureDetector(
                                    onTap: () async {
                                      ...
                                      final meaning = currentName['meaning'];
                                      //await DatabaseHelper().updateLikedName(id, 1);
                                      await _saveRejectdNameToFirestore(name, meaning);
                                      setState(() {
